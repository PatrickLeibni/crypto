...
import hashlib
import secrets
from src.sm2_signature_misuse import SM2SignatureMisuse
# 移除不存在的函数导入
...
class TestSignatureMisuse(unittest.TestCase):
    ...
    def test_key_recovery_attack(self):
        ...
        # 尝试恢复私钥
        recovered_key = self.misuse._recover_private_key_from_nonce_reuse(
    ...
    def test_poc_demonstrations(self):
        ...
        # 演示随机数重用攻击
        print("\n=== 随机数重用攻击演示 ===")
        from src.sm2_misuse_poc import SM2MisusePOC
        poc = SM2MisusePOC()
        poc.nonce_reuse_attack_demo()
        
        # 演示签名可延展性攻击
        print("\n=== 签名可延展性攻击演示 ===")
        poc.signature_malleability_attack_demo()
        
        # 演示重放攻击
        print("\n=== 重放攻击演示 ===")
        poc.replay_attack_demo()
