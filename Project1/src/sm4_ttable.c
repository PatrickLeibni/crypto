#include "sm4_ttable.h"
#include <string.h>

// SM4 S-box 
static const uint8_t SM4_SBOX[256] = {
    
};

static uint32_t rotl(uint32_t x, int n) {
    return (x << n) | (x >> (32 - n));
}

// SM4 FK常量
static const uint32_t SM4_FK[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

// SM4 CK常量
static const uint32_t SM4_CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

static void sm4_set_key(const uint8_t* key, uint32_t* rk) {
    uint32_t mk[4];
    uint32_t k[36];
    
    // 将密钥转换为32位字
    for (int i = 0; i < 4; i++) {
        mk[i] = ((uint32_t)key[4*i] << 24) | ((uint32_t)key[4*i+1] << 16) |
                 ((uint32_t)key[4*i+2] << 8) | ((uint32_t)key[4*i+3]);
    }
    
    // 密钥扩展
    for (int i = 0; i < 4; i++) {
        k[i] = mk[i] ^ SM4_FK[i];
    }
    
    for (int i = 0; i < 32; i++) {
        uint32_t temp = k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ SM4_CK[i];
        
        // T'函数
        uint8_t b[4];
        for (int j = 0; j < 4; j++) {
            b[j] = (temp >> ((3-j)*8)) & 0xff;
        }
        uint32_t sbox_out = ((uint32_t)SM4_SBOX[b[0]] << 24) | 
                            ((uint32_t)SM4_SBOX[b[1]] << 16) | 
                            ((uint32_t)SM4_SBOX[b[2]] << 8) | 
                            SM4_SBOX[b[3]];
        uint32_t L_prime = sbox_out ^ rotl(sbox_out, 13) ^ rotl(sbox_out, 23);
        
        k[i + 4] = k[i] ^ L_prime;
        rk[i] = k[i + 4];
    }
}

void sm4_encrypt_ttable(const uint8_t *key, const uint8_t *plaintext, uint8_t *ciphertext) {
    uint32_t rk[32];
    sm4_set_key(key, rk);
    
    uint32_t X[4];
    for(int i = 0; i < 4; ++i) {
        X[i] = ((uint32_t)plaintext[4*i] << 24) | ((uint32_t)plaintext[4*i+1] << 16) | 
                ((uint32_t)plaintext[4*i+2] << 8) | plaintext[4*i+3];
    }
    
    for(int r = 0; r < 32; ++r) {
        uint32_t T_arg = X[1] ^ X[2] ^ X[3] ^ rk[r];
        
        // T函数计算
        uint8_t b[4];
        for(int i = 0; i < 4; ++i) {
            b[i] = (T_arg >> ((3 - i) * 8)) & 0xFF;
        }
        uint32_t sbox_out = ((uint32_t)SM4_SBOX[b[0]] << 24) | 
                            ((uint32_t)SM4_SBOX[b[1]] << 16) | 
                            ((uint32_t)SM4_SBOX[b[2]] << 8) | 
                            SM4_SBOX[b[3]];
        uint32_t T_out = sbox_out ^ rotl(sbox_out, 2) ^ rotl(sbox_out, 10) ^ 
                         rotl(sbox_out, 18) ^ rotl(sbox_out, 24);
        
        X[0] ^= T_out;
        
        // 循环左移
        uint32_t temp = X[0];
        X[0] = X[1]; X[1] = X[2]; X[2] = X[3]; X[3] = temp;
    }
    
    for(int i = 0; i < 4; ++i) {
        ciphertext[4*i]   = (X[3-i] >> 24) & 0xFF;
        ciphertext[4*i+1] = (X[3-i] >> 16) & 0xFF;
        ciphertext[4*i+2] = (X[3-i] >> 8) & 0xFF;
        ciphertext[4*i+3] = X[3-i] & 0xFF;
    }
}

void sm4_decrypt_ttable(const uint8_t *key, const uint8_t *ciphertext, uint8_t *plaintext) {
    uint32_t rk[32];
    sm4_set_key(key, rk);
    
    // 解密时轮密钥顺序相反
    uint32_t rk_reverse[32];
    for(int i = 0; i < 32; ++i) {
        rk_reverse[i] = rk[31 - i];
    }
    
    uint32_t X[4];
    for(int i = 0; i < 4; ++i) {
        X[i] = ((uint32_t)ciphertext[4*i] << 24) | ((uint32_t)ciphertext[4*i+1] << 16) | 
                ((uint32_t)ciphertext[4*i+2] << 8) | ciphertext[4*i+3];
    }
    
    for(int r = 0; r < 32; ++r) {
        uint32_t T_arg = X[1] ^ X[2] ^ X[3] ^ rk_reverse[r];
        
        // T函数计算
        uint8_t b[4];
        for(int i = 0; i < 4; ++i) {
            b[i] = (T_arg >> ((3 - i) * 8)) & 0xFF;
        }
        uint32_t sbox_out = ((uint32_t)SM4_SBOX[b[0]] << 24) | 
                            ((uint32_t)SM4_SBOX[b[1]] << 16) | 
                            ((uint32_t)SM4_SBOX[b[2]] << 8) | 
                            SM4_SBOX[b[3]];
        uint32_t T_out = sbox_out ^ rotl(sbox_out, 2) ^ rotl(sbox_out, 10) ^ 
                         rotl(sbox_out, 18) ^ rotl(sbox_out, 24);
        
        X[0] ^= T_out;
        
        // 循环左移
        uint32_t temp = X[0];
        X[0] = X[1]; X[1] = X[2]; X[2] = X[3]; X[3] = temp;
    }
    
    for(int i = 0; i < 4; ++i) {
        plaintext[4*i]   = (X[3-i] >> 24) & 0xFF;
        plaintext[4*i+1] = (X[3-i] >> 16) & 0xFF;
        plaintext[4*i+2] = (X[3-i] >> 8) & 0xFF;
        plaintext[4*i+3] = X[3-i] & 0xFF;
    }
} 
