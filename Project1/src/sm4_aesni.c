...
// SM4密钥扩展
void sm4_key_expansion_aesni(const uint8_t *key, uint32_t *rk) {
    uint32_t mk[4];
    uint32_t k[36];
...
// 使用AESNI的SM4加密/解密
void sm4_crypt_aesni(const uint8_t *input, uint8_t *output, const uint32_t *rk, int encrypt) {
    uint32_t x[4];
    ...
    for (int i = 0; i < 32; i++) {
        ...
        uint32_t temp = x[0] ^ sm4_t_aesni(x[1] ^ x[2] ^ x[3] ^ rk[rk_idx]);
        x[0] = x[1];
        x[1] = x[2];
        x[2] = x[3];
        x[3] = temp;
...
void sm4_encrypt_aesni(const uint8_t *key, const uint8_t *plaintext, uint8_t *ciphertext) {
    uint32_t rk[32];
    sm4_key_expansion_aesni(key, rk);
    sm4_crypt_aesni(plaintext, ciphertext, rk, 1);
}
...
void sm4_decrypt_aesni(const uint8_t *key, const uint8_t *ciphertext, uint8_t *plaintext) {
    uint32_t rk[32];
    sm4_key_expansion_aesni(key, rk);
    sm4_crypt_aesni(ciphertext, plaintext, rk, 0);
}
