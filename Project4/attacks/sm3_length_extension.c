...
void create_padding(uint8_t *padding, size_t original_len, size_t *padding_len) {
    ...
    size_t blocks_needed = (total_len + 9 + 63) / 64;  // 向上取整
...
int sm3_length_extension_attack(const uint8_t *original_digest,
    ...
    // 将原始摘要转换为状态
    uint32_t state[8];
    digest_to_state(original_digest, state);
    
    // 初始化上下文
    sm3_ctx_t ctx;
    memcpy(ctx.state, state, sizeof(state));
    ctx.length = original_len * 8 + padding_len * 8;  // 已经处理的比特数
    ctx.buffer_size = 0;
...
int sm3_verify_length_extension_attack(const uint8_t *original_message, 
    ...
    uint8_t combined_message[1024];  // 根据需要调整大小
