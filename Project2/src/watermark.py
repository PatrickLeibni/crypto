...
class DigitalWatermark:
    ...
    def embed_watermark(
        ...
        cr_channel = cr_channel[:h, :w]
        cb_channel = cb_channel[:h, :w]
        ...
        # 嵌入水印
        watermarked_y = self._embed_dct(y_channel, watermark)

        # 保存结果
        if output_path:
            # 将嵌入后的 Y 与原始色度通道合并回彩色图，保持颜色不变
            merged_ycrcb = cv2.merge([watermarked_y, cr_channel, cb_channel])
            watermarked_bgr = cv2.cvtColor(merged_ycrcb, cv2.COLOR_YCrCb2BGR)
            cv2.imwrite(output_path, watermarked_bgr)

        # 返回用于质量评估的亮度通道（原始与嵌入后）
        return y_channel, watermarked_y
    ...
    def extract_watermark(
        ...
        # 提取水印
        extracted_watermark = self._extract_dct(y_channel, watermark_size)
    ...
    def detect_watermark(
        ...
        threshold: float = 0.5,  # 添加阈值参数，默认值0.5
        seed: int = 42,  # 添加种子参数，默认值42
    ) -> Tuple[bool, float]:
        """
        检测图片中的水印
        ...
        extracted_watermark = self.extract_watermark(
            image_path, original_size, watermark_size, seed=seed  # 传递seed参数
        )

        # 生成期望的水印
        expected_watermark = self._generate_watermark(watermark_size, expected_message)

        # 计算相似度
        # 确保数组大小一致
        min_length = min(len(extracted_watermark), len(expected_watermark))
        extracted_watermark = extracted_watermark[:min_length]
        expected_watermark = expected_watermark[:min_length]
        similarity = np.mean(extracted_watermark == expected_watermark)

        return similarity > threshold, similarity  # 使用threshold参数
        """
        检测图片中是否存在水印
