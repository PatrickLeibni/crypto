...
def create_sample_image(
    output_path: str = "outputs/sample_image.jpg", size: tuple = (512, 512)
):
    """
    创建示例图片用于测试
...
def demonstrate_watermarking():
    ...
    # 确保中文字体
    chosen_font = configure_chinese_fonts()
    if chosen_font:
        print("已启用中文字体:", chosen_font)
    else:
        print("未找到中文字体，已设置候选字体列表作为回退")

    # 创建示例图片
    sample_image = create_sample_image()

    # 初始化水印系统
    watermark_system = DigitalWatermark(alpha=0.3)  # 提高水印强度从0.15到0.3
    ...
    original_img, watermarked_img = watermark_system.embed_watermark(
        ...
        output_path=str(OUTPUTS_DIR / "watermarked_image.jpg"),
    )

    print("水印嵌入完成!")
    print("含水印图片已保存为:", str(OUTPUTS_DIR / "watermarked_image.jpg"))
    ...
    detected, similarity = watermark_system.detect_watermark(
        str(OUTPUTS_DIR / "watermarked_image.jpg"),
        original_size=(h, w),
...
def demonstrate_robustness_testing(
    ...
    # 生成报告
    tester.generate_report(str(OUTPUTS_DIR / "robustness_report.txt"))
    tester.plot_results(str(OUTPUTS_DIR / "robustness_plot.png"))
...
def visualize_results():
    ...
    # 读取图片
    original = cv2.imread(str(OUTPUTS_DIR / "sample_image.jpg"))
    watermarked = cv2.imread(str(OUTPUTS_DIR / "watermarked_image.jpg"))
    ...
    if original is not None and watermarked is not None:
        ...
        plt.savefig(str(OUTPUTS_DIR / "watermark_comparison.png"), dpi=300, bbox_inches="tight")
        plt.close()

        print("对比图已保存为:", str(OUTPUTS_DIR / "watermark_comparison.png"))
...
def main():
    ...
    try:
        ...
        print("\n演示完成!")
        print("生成的文件:")
        print("- outputs/sample_image.jpg (原始图片)")
        print("- outputs/watermarked_image.jpg (含水印图片)")
        print("- outputs/watermark_comparison.png (对比图)")
        print("- outputs/robustness_report.txt (测试报告)")
        print("- outputs/robustness_plot.png (测试结果图表)")
