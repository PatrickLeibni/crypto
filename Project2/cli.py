...
from config import WATERMARK_CONFIG, OUTPUTS_DIR
...
def embed_watermark(args):
    ...
    if not input_path.exists():
        # 若相对路径未找到，尝试在 outputs 目录下寻找
        if not input_path.is_absolute():
            candidate = OUTPUTS_DIR / input_path
            if candidate.exists():
...
def test_robustness(args):
    ...
    try:
        ...
        # 生成报告与图表（相对路径统一写入 outputs/）
        if args.report:
            report_path = Path(args.report)
            if not report_path.is_absolute():
                report_path = OUTPUTS_DIR / report_path
        ...
        if args.plot:
            ...
            if not plot_path.is_absolute():
                plot_path = OUTPUTS_DIR / plot_path
            plot_path.parent.mkdir(parents=True, exist_ok=True)
            tester.plot_results(str(plot_path))
            logger.info(f"测试图表已保存到: {plot_path}")
...
示例用法:
  ...
  %(prog)s create-sample -o sample.jpg
    ...
    embed_parser.add_argument("-o", "--output", required=True, help="输出图片路径（默认写入 outputs/ 下，除非提供绝对路径）")
